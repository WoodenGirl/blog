<template>
  ...
  >
    <template #default="{ node, data }">
        <span class="custom-tree-node" @click="toList(data.id)">
...
const toList = (id: number) => {
  articlesStore.clickCategory(id)
...
const remove = (data: Tree) => {
  ElMessageBox.confirm(
    ...
    .then(() => {
      const newChild = {categoryId: data.id}
      deleteCategory(newChild).then(() => {
        fetchTreeData()
        ElMessage({
          type: 'success',
